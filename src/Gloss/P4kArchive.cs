...
/// ordinary ZIP readers is the payload codec: entries use ZStd under method 100,
...
/// reverse-engineering behind scdatatools and the unp4ck tools before it. This
/// reader is ported from Quantum Wake's, which is our own implementation of it.
/// </para>
/// <para>
/// Nothing is unpacked to disk: one entry is located and decompressed in memory.
/// The file is opened read-only and shared, so it is safe to do while the game
/// is running.
/// </para>
/// </remarks>
public sealed class P4kArchive(string path)
{
    ...
    /// <summary>Extracts one entry by full path.</summary>
    ...
    public byte[]? TryRead(string entryPath)
    {
        ...
        // A ZStd frame starts 28 B5 2F FD. Anything else here is encrypted, and
        // guessing at it would produce confident nonsense.
    ...
    private static (long Offset, long Count) FindCentralDirectory(FileStream stream, BinaryReader reader)
    {
        ...
        // Sentinels mean the real values are in the ZIP64 record, which at this
        // archive's size is always the case.
        ...
        FindEntry(FileStream stream, BinaryReader reader, long directoryOffset, long entryCount, string wanted)
    {
        ...
        var normalised = wanted.Replace('/', Path.DirectorySeparatorChar);
        ...
        for (long i = 0; i < entryCount; i++)
        {
            ...
            if (!name.Replace('/', Path.DirectorySeparatorChar)
                     .Equals(normalised, StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
